...
async def gemini_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(word in lowered for word in ["سلام", "درود"]):
        await update.message.reply_text(f"سلام {user_name} عزیز! :tulip:")
        return
    elif any(word in lowered for word in ["خدافظ", "خداحافظ", "بای"]):
        await update.message.reply_text(f"خداحافظ {user_name} نازنین! :wave:")
        return
    elif any(word in lowered for word in ["خوبی", "چطوری", "حالت چطوره"]):
        await update.message.reply_text(f"مرسی {user_name} جان، من خوبم :blush: تو چطوری؟")
        return

    await update.message.reply_text("🤖 در حال فکر کردنه... لطفاً صبر کن...")
    ...
    try:
        response = model.generate_content([
            {"role": "user", "parts": ["تو یک دستیار فارسی‌زبان هستی. شوخ‌طبع باش و از ایموجی استفاده کن. :blush:"]}
        ] + history)
        reply = response.text
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("🖼 در حال بررسی تصویر هستم...")
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        ogg_path = await file.download_to_drive()
        ...
        await update.message.reply_text(f":x: خطا در تحلیل ویس: {str(e)}")
...
print(":white_check_mark: Parsify Webhook bot is ready!")
